...
import LitigationEngagementPreview from '../components/AgreementPreview/LitigationEngagementPreview';
...
export default function LitigationEngagementPreviewPage() {
  const location = useLocation();
  const navigate = useNavigate();
  const { type } = useParams(); // 'litigation-engagement'
  const formType = type as FormType;

  const formData = location.state?.formData as LitigationEngagementFormData;
